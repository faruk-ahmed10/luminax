...
const Spot = () => {
    ...
    const { selectedAccount, claimsAvailable, payoutOf, totalWithdraw, userInfo, userInfoTotals, fromWei } = useGlobalContext();
    ...
    const spotItems = [
        ...
        {
            title: 'Deposit',
            token_amount: usrTtlInfo ? Number(fromWei(usrTtlInfo.total_deposits)).toFixed(6) : 0,
            balance: 10098.36
        },
        {
            title: 'Drained',
            token_amount: usrTtlInfo ? Number(totWithdraw).toFixed(6) : 0,
            balance: 10098.36
        },
        {
            title: 'Max Payout',
            token_amount: mPayout ? Number(mPayout).toFixed(6) : 0,
            balance: 10098.36
        },
        {
            title: 'Rewarded',
    ...
    useEffect(() => {
        ...
        payoutOf().then(mp => setMPayout(fromWei(mp.max_payout))).catch(err => console.log(err));

        userInfo().then(ui => setusrInfo(ui)).catch(err => console.log("User info", err));

        totalWithdraw().then(withdraw => setTotWithdraw(fromWei(withdraw))).catch(err => console.log("Total Withdraw: ", err))
    }, [selectedAccount]);

    useEffect(() => {
        if (!selectedAccount) return;
        userInfoTotals(selectedAccount).then(ui => setUsrTtlInfo(ui)).catch(err => console.log("User info", err));
